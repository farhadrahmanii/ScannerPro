    ...
    <div class="p-4 card-body">
        ...
        <form class="row g-3" wire:submit.prevent="save" method="POST" id="myForm" enctype="multipart/form-data"
            ...
            <div class="form-group col-md-3">
                ...
                <select class="form-control rounded-lg @error('provinces_id') is-invalid @enderror"
                    wire:model="provinces_id" name="provinces_id" id="provinces_id">
                    <option value="">Select Province</option>
                    @foreach ($provinces as $province)
                        <option value="{{ $province->id }}" @if($province->id == $provinces_id) selected @endif>
            ...
            <div class="form-group col-md-3">
                ...
                <select id="site_manager" wire:model="site_manager"
                    ...
                    @foreach ($users as $u)
                        <option value="{{ $u->id }}" @if($u->id == $site_manager) selected @endif>
                            {{ $u->name }}
                        </option>
                    @endforeach
                </select>
